	...
	<div class="col m12 s12">
		<div class="card">
			<div class="card-content">
				...
				<p>Disini anda bisa merubah soal ujian berbentuk pilihan ganda, silahkan masukan pertanyaan, pilihan jawaban beserta kunji jawabannya.</p>
			</div>
		</div>
	</div>
</div>
					...
					<div class="col m6 s6">
						...
						if (($quiz->audio != NULL) AND ($quiz->audio != '')) {
							if (file_exists('./aila_cbt/audio/'.$quiz->quiz_name_ID.'/'.$quiz->audio)) {
								...
								echo "<source src='../../../aila_cbt/audio/".$quiz->quiz_name_ID."/".$quiz->audio."' type='audio/mpeg'>";
								echo "<source src='".base_url('aila_cbt/audio/'.$quiz->quiz_name_ID."/".$quiz->audio)."' type='audio/mpeg'>";
								echo "</audio>";
								echo "</div>";
								echo "<div class='col m6 s6'>";
								echo "<br/><a onclick='return confirm(\"Apakah mau audio tersebut?\")' href='".base_url('teacher/quiz/delete_audio/'.$quiz->audio.'/'.$ID)."' class='btn red'>Hapus Audio</a>";
								echo "</div></div>";
							}
						}?>
					</div>
