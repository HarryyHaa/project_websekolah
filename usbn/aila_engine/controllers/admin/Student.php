	...
	public function create()
	{
		if (($this->input->post()) && (!empty($this->input->post()))) 
		{
			...
			if ($validation->run()) {
				...
				{
					$this->session->set_flashdata('failed', 'Maaf, siswa dengan NIS tersebut sudah terdaftar!');
				}
				else
				{
					// Create new student
	...
	public function import()
	{
		...
		$highestColumn = $sheet->getHighestColumn();
            ...
            // Cek apakah NIM sudah ada atau belum, kalau ada maka lakukan proses update kalau belum ada maka lakukan proses insert
					...
					$cek_nim = $this->student_model->get_student_code($rowData[0][0]);
					if ($cek_nim >=1 ) {
						$this->student_model->update_data($where,$data);
					}
					else
					{
                //sesuaikan nama dengan nama tabel
						$insert = $this->student_model->create_data($data);
					}
				}
			...
			public function update($id = NULL)
			{
				...
				// If student code has been registered
						...
						{
							$this->session->set_flashdata('failed', 'Maaf, NIP tidak boleh sama dengan siswa lain!');
						}
						else
						{
					// Create new student
