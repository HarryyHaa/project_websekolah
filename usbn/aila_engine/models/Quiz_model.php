...
class Quiz_model extends CI_Model
{
	...
	public function get_quiz_rand_by_quiz_name_id($id, $limit = NULL, $quiz_type = '1', $student_ID, $classroom_ID, $random_number)
	{
	...
	{
		return $this->db
		->select('a.answer, a.is_correct, b.question, b.answer_key, b.answer_1, b.answer_2, b.answer_3, b.answer_4, b.answer_5, b.explanation')
		->from('quiz_answer a')
	...
	{
		return $this->db
		->select('a.ID, a.answer_essay, a.answer_score, b.question')
		->from('quiz_answer a')
		->join('quiz b', 'b.ID=a.quiz_ID')
		->where('a.classroom_ID', $classroom_ID)
		->where('a.student_ID', $student_ID)
	...
	{
		return $this->db
		->select('a.ID, a.quiz_ID, a.number, a.classroom_ID, a.answer, a.answer_essay, b.quiz_type, b.question, b.audio, b.quiz_name_ID, b.answer_1, b.answer_2, b.answer_3, b.answer_4, b.answer_5, is_doubtful')
		->from('quiz_answer a')
	...
	public function get_quiz_total($classroom_ID, $student_ID)
	{
		return $this->db
		->select('ID')
		->where('classroom_ID', $classroom_ID)
		->where('student_ID', $student_ID)
		->get($this->_table_quiz_answer)
		->num_rows();
	}

	public function count_mutiple_choice_by_cc($classroom_code)
	{
		$query1 =  $this->db
		...
		else
		{
			...
			->join('classroom b', 'b.ID=a.classroom_ID')
			...
			if ($query2->num_rows() > 0) {
				$student_ID = $query2->row()->student_ID;
			}else{
				$student_ID = 0;
			}
	...
	public function count_quiz_by_classroom_id($classroom_ID, $quiz_type = NULL)
	{
		if ($quiz_type == 1) {
			...
			if ($multiple_choice_limit > 0 ) {
				...
				$this->db->from('quiz_answer a');
				$this->db->join('quiz c', 'c.ID=a.quiz_ID', 'left');
				$this->db->join('classroom b', 'b.quiz_name_ID=c.quiz_name_ID', 'left');
				$this->db->where('a.classroom_ID', $classroom_ID);
				if ($quiz_type != NULL) {
					$this->db->where('c.quiz_type', $quiz_type);
				}
				...
				$this->db->select('a.quiz_ID, a.classroom_ID');
				$this->db->join('quiz c', 'c.ID=a.quiz_ID', 'left');
				$this->db->where('a.classroom_ID', $classroom_ID);
				if ($quiz_type != NULL) {
					$this->db->where('c.quiz_type', $quiz_type);
				...
				$this->db->group_by('a.classroom_ID');
				$query3 = $this->db->get('quiz_answer a')->num_rows();
	...
	{
		return $this->db
		->select('a.ID, a.answer_essay, answer_score, b.question')
		->from('quiz_answer a')
		->join('quiz b', 'b.ID=a.quiz_ID')
	...
	{
		return $this->db
		->select('SUM(answer_score) as jumlah')
		->from('quiz_answer')
		->where('classroom_ID', $classroom_ID)
		->get()
		->row()
