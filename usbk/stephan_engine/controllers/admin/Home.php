...
class Home extends CI_Controller
{
	...
	public function get_total_teacher($status = 1)
	{
		echo row_count('teacher', 'ID', ['status' => $status]);
	}

	public function get_total_student($status = 1)
	{
		echo row_count('student', 'ID', ['status' => $status]);
	}

	public function get_total_classroom($status = 1)
	{
		echo row_count('classroom', 'ID', ['status' => $status]);
	}

	public function get_total_quiz_name($status = 1)
	{
		echo row_count('quiz_name', 'ID', ['status' => $status]);
	}
