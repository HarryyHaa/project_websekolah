...
class Quiz extends CI_Controller {
	...
	{
		$this->template->view('quiz_name');
	}

	public function get_quiz_name_json($status=1)
	{
		$quiz_name =  $this->quiz_model->get_quiz_name_with_datatables($status,  $this->session->userdata['ID']);
		$data = array();
		...
		foreach ($quiz_name as $quiz) 
		{
			...
			$row[] = "<a href=\"".site_url('teacher/quiz/questions_list/'.encode($quiz->ID))."\" class=\"btn-small blue\">Selengkapnya</a>";
			
			$row[] = "<a  href=\"".site_url('teacher/quiz/update_quiz_name/').encode($quiz->ID)."\" class=\"btn-small green modal-trigger\">Ubah</a>
			<a onclick=\"return confirm('Apakah mau mengarsipkan quiz ini beserta seluruh pertanyaannya?');\" href=\"".site_url('teacher/quiz/make_an_archive/'.encode($quiz->ID))."\" class=\"btn-small orange\">Arsipkan</a>
			<a target=\"_blank\" class=\"btn-small blue\" href=\"".site_url('teacher/quiz/export_question/'.encode($quiz->ID));"\">Eksport Soal</a>";
		...
		$output = array(
			...
			"recordsTotal" => $this->quiz_model->count_all($status,  $this->session->userdata['ID']),
			"recordsFiltered" => $this->quiz_model->count_filtered($status,  $this->session->userdata['ID']),
			"data" => $data,
		);
	...
	public function get_quiz_name_archive_json($status=2)
	{
		...
		foreach ($quiz_name as $quiz) 
		{
			...
			$row[] = "<a onclick=\"return confirm('Apakah mau mengaktifkan paket soal ini beserta seluruh pertanyaannya?');\" href=\"".site_url('teacher/quiz/make_active/'.encode($quiz->ID))."\" class=\"btn-small green\">Aktifkan</a>
			<a target=\"_blank\" class=\"btn-small blue\" href=\"".site_url('teacher/quiz/export_question/'.encode($quiz->ID))."\">Eksport Soal</a>
			<a onclick=\"return confirm('Apakah Anda yakin akan menghapus permanen paket soal ini beserta seluruh data yang terkait?');\" href=\"".site_url('teacher/quiz/confirm_delete_quiz_name/'.encode($quiz->ID))."\" class=\"btn-small red\">Hapus Permanen</a>";
...
public function make_active($id = NULL)
{
	...
	redirect('teacher/quiz/quiz_name_archive');
}
...
{
	if(!$code) redirect('teacher/quiz/quiz_name_archive');
