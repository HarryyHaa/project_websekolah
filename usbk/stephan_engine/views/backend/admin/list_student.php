		...
		<div class="card">
			<div class="card-content">
				...
				<h5>Pilih <?=$this->config->item('student');?> <?=(($group_name != NULL) ? $group_name : '');?> &nbsp; <a href="#filter_group" class="btn-small green modal-trigger">Filter <i class="material-icons right" title="Filter Jurusan">filter_list</i></a> <button onclick="return confirm('Apakah Anda yakin akan memasukan <?=$this->config->item('student');?> tersebut ke kelas ujian? Proses ini akan membutuhkan waktu beberapa saat sesuai dengan jumlah <?=$this->config->item('student');?> yang dipilih')" type='submit' class="btn blue">Proses</button> 
						<a class="btn green" href="<?=site_url('admin/classroom/check_code/'.$classroom->code);?>">Kembali</a></h5>
					<table class="table">
						<thead>
							<tr>
								<th><label class="left"><input type="checkbox" id="select-all" name="select-all" /><span></span></label>Pilih Semua</th>
								<th>No.</th>
						...
						<a class="btn green" href="<?=site_url('admin/classroom/check_code/'.$classroom->code);?>">Kembali</a>
					</form>
				</div>
			</div>
		</div>
...
<div id="filter_group" class="modal">
	<div class="modal-content">
		<h4>Pilih Jurusan </h4>
			<a href="<?=site_url('admin/classroom/un_filter_group/'.encode($classroom->ID));?>">
				<div class="card">
		...
		<?php foreach ($groups as $group) {?>
			<a href="<?=site_url('admin/classroom/filter_group/'.$group->ID.'/'.encode($classroom->ID));?>">
				<div class="card">
					<div class="card-content center">
