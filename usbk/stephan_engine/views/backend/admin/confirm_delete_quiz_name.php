					...
					<tbody>
						<?php foreach ($classroom->result() as $row) { ?>
							<tr>
								...
								<td> <a onclick="return confirm('Apakah yakin kelas ini mau dihapus permanen?')" href="<?=site_url('admin/classroom/delete_archive/'.$row->code.'/'.encode($quiz_name->ID));?>" class="btn-small red" title="Arsipkan Kelas">Hapus Permanen</a></td>
							</tr>
						<?php }?>
					</tbody>
				</table>
