		...
		<div class="col m12 s12">
			<div class="card">
				<div class="card-content">
					<h5> <center><?=$this->config->item('student');?></center><br/>
						<center>
						...
						<a href="<?=site_url('teacher/classroom/detail_classroom/'.strtoupper($classroom->code));?>" class="btn-small green" title="Daftar Nilai">Nilai<i class="material-icons right">list</i></a>
					...
					<table id="dataTables">
						<thead>
							<tr>
								...
								<th>Nama</th>
								<th class="center">Mulai Mengerjakan</th>
								<th>Perangkat</th>
								<th>IP</th>
								<th>Browser</th>
								<th>Reset</th>
						...
						<tbody>
							<?php $no = 1; foreach ($present as $row) {?>
								<tr>
									...
									<td><?=$row->code;?></td>
									<td><?=$row->name;?></td>
									<td class="center"><?=($row->start_time != NULL) ? bulan_indo($row->start_time).' '.date('H:i', $row->start_time). ' WIB' : 'Belum mulai';?></td>
									<td><?=$row->device;?></td>
									<td><?=$row->ip;?></td>
									<td><?=$row->user_agent;?></td>
									<td><?="<a title=\"Reset\" onclick=\"return confirm('Apakah Anda yakin ingin mereset waktu ujian beserta jawaban untuk peserta tersebut?')\" href=\"".site_url('teacher/classroom/reset/'.encode(encode($row->ID).'/'.encode($classroom->ID).'/'.encode($classroom->code)))."\" class=\"btn btn-small red\">Reset</a>";?></td>
									<?php $no++; }?>
								</tbody>
