					...
					<div class="col m6">
						<table>
							...
							<tr>
								<td>Nilai PG</td>
								<td>:</td>
								<td><?=($total_mc_quiz!= 0 ?(number_format((($correct_answer / $total_mc_quiz) * 100), 2)) : 0);?></td>
							</tr>
							<tr>
								<td>Nilai Essai</td>
								<td>:</td>
				...
				<table class="table responsive-table">
					<thead>
						<tr>
							...
							<?=(($total_mc_quiz != 0) ? "<th>Jawaban Benar</th>" : '');?>
							<th>Pembahasan</th>
						</tr>
					</thead>
					<tbody>
						<?php $no=1; foreach ($multiple_choice as $row) { ?>
							...
							<?php $no=$no; foreach ($essay as $row2) { ?>
								<tr>
									...
									<td>
									<?php if ($row2->explanation != NULL) {?>
										<a href="<?=site_url('student/classroom/explanation/'.encode($row2->ID));?>" class="btn btn-small green">Lihat</a>
									<?php }else{echo "-";}; ?>
								</td>
								</tr>
								<?php $no++; }?>
